...
public class TrailPaint : MonoBehaviour
{
    ...
    void DrawSnowMap(float posX, float posY)
    {
        ...
        // draw a black spot on the contact place)
        Graphics.DrawTexture(new Rect(posX - splash.width / splashSize, (rendText.height - posY) - splash.height / splashSize, splash.width / (splashSize * 0.4f), splash.height / (splashSize * 0.4f)), splash);
        GL.PopMatrix();
